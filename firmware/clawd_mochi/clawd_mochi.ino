...
// ── Claude Code usage received over USB serial ───────────────
uint8_t usageWeekly = 0;
uint8_t usageFiveHour = 0;
String usageWeeklyReset;
...
void drawUsageBar(int16_t y, uint8_t percentage) {
  const int16_t x = 12;
  const int16_t w = DISP_W - 24;
  const int16_t h = 14;
  const uint16_t fill = ((uint32_t)(w - 2) * percentage) / 100;
  const uint16_t colour = percentage >= 80 ? C_ORANGE : C_GREEN;
...
void drawUsageView() {
  ...
  tft.setCursor(12, 14); tft.print("CLAUDE USAGE");
  tft.setTextColor(C_GREEN); tft.setTextSize(1);
  tft.setCursor(160, 39); tft.print("USB-C LIVE");

  tft.setTextColor(C_WHITE); tft.setTextSize(2);
  tft.setCursor(12, 54); tft.print("WEEKLY");
  tft.setCursor(166, 54); tft.print(usageWeekly); tft.print("%");
  drawUsageBar(78, usageWeekly);
  tft.setTextColor(C_MUTED); tft.setTextSize(1);
  tft.setCursor(12, 98); tft.print("resets "); tft.print(usageWeeklyReset);

  tft.setTextColor(C_WHITE); tft.setTextSize(2);
  tft.setCursor(12, 132); tft.print("5 HOUR");
  tft.setCursor(166, 132); tft.print(usageFiveHour); tft.print("%");
  drawUsageBar(156, usageFiveHour);
  tft.setTextColor(C_MUTED); tft.setTextSize(1);
  tft.setCursor(12, 176); tft.print("resets "); tft.print(usageFiveHourReset);
...
void updateAutoRotation() {
  ...
  // The second eye view changes state several times during its five seconds.
  if (autoPhase == AUTO_MOVING_EYES && now - lastEyeFrameAt >= EYE_FRAME_MS) {
    lastEyeFrameAt = now;
    const bool closed = ((now - phaseStartedAt) / EYE_FRAME_MS) % 2;
    drawSquishEyes(closed);
  }
}
...
// The Claude Code status-line script sends this over USB-C serial:
// USAGE|<weekly percent>|<five-hour percent>|<weekly reset>|<five-hour reset>
void applyUsageLine(String line) {
  line.trim();
  if (!line.startsWith("USAGE|")) return;
  ...
  const uint8_t weekly = constrain(line.substring(p1 + 1, p2).toInt(), 0, 100);
  const uint8_t fiveHour = constrain(line.substring(p2 + 1, p3).toInt(), 0, 100);
...
// only display controls; the ESP32 never needs an IP address for this path.
void applyUsbCommand(String line) {
  if (!line.startsWith("CMD|")) return;

  const String command = line.substring(4);
  if (command == "EYES") {
    currentView = VIEW_EYES_NORMAL;
    animNormalEyes();
...
void handleUsageSerial() {
  static String line;
  while (Serial.available()) {
    const char c = (char)Serial.read();
    if (c == '\n') {
      if (line.startsWith("USAGE|")) applyUsageLine(line);
      else applyUsbCommand(line);
      line = "";
